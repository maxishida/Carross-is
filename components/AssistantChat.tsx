...
const AssistantChat: React.FC<AssistantChatProps> = ({ isOpen, onClose, onApplyIdea }) => {
    ...
    return (
        <div 
            className={`fixed inset-y-0 right-0 z-50 w-full sm:w-[350px] bg-white dark:bg-[#0f111a] border-l border-gray-200 dark:border-white/10 shadow-2xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
            {/* Header */}
            <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-white/10 bg-white/50 dark:bg-black/20 backdrop-blur-md">
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 h-[calc(100vh-130px)] custom-scrollbar bg-slate-50/50 dark:bg-[#0f111a]">
                {messages.map((msg) => (
                    <div 
                        key={msg.id} 
            ...
            {/* Input Area */}
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-white dark:bg-[#0f111a] border-t border-gray-200 dark:border-white/10">
